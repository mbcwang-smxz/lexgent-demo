...
*   **工作流引擎 (Workflow Engine)**: 支持定义包含多个顺次 LLM 调用或工具操作的复杂技能。
*   **标准化上下文**: 上下文 (Context) 由 Case Server 统一管理，支持断点续传和多端协同。
...
graph TD
    ...
    AgentServer <--> CaseServer[Case Server]
...
6.  **反馈**: 结果写入 Case Server，并通过 Callback 通知 Client。
...
*   [Case Server API](./api_case_server.md)
