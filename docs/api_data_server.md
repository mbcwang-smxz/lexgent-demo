...
**核心职责**：所有案件数据（原始证据、AI 生成文件、执行日志）都通过 Data Server 统一管理。Agent Framework 通过 `ICaseDataStore` 接口与之交互，不直接操作文件系统。
...
**功能**：直接返回 `metadata.json` 的原始内容，**不执行**物理文件同步（与 `GET /context` 的区别）。

**重要性**：`Skill_初始化`（文档分类）的 tasks 管道中通过 `api_request` 调用此端点获取当前 metadata，供 LLM 在分类时参考已有文件注册信息。Engine 的 `ContextInitializer` 也通过此端点读取 metadata 进行增量更新。
...
// 响应 200
{
  "caseId": "case-101",
  "files": [...],
  "metadata": { "caseNumber": "case-101" }
}
```

### 2.4 替换元数据

**PUT** `/cases/:caseId/metadata`

**功能**：整体替换 `metadata.json`。服务器自动确保 `caseId` 字段与路径参数一致。

**重要性**：Engine 的 `ContextInitializer` 在完成文件转换注册、分类合并等操作后，通过此端点将更新后的完整 metadata 写回。`Skill_初始化` 的分类结果也经此端点持久化。
...
> **`GET /context` vs `GET /metadata` vs `PUT /metadata`**：
> - `GET /context`：读取 + 双向同步（自动发现新文件、清理已删除文件）
> - `GET /metadata`：只读，不触发同步
> - `PUT /metadata`：整体替换（ContextInitializer / 分类结果写回）
...
**GET** `/cases/:caseId/files/scan`

**功能**：扫描案件目录中的物理文件，返回文件名、大小、修改时间。可选附带内容预览（前 300 字符）。与 `GET /context` 不同，此 API 直接读取磁盘，不经过 metadata 注册表。

**重要性**：`Skill_初始化`（文档分类）在 LLM 分类前调用此 API（`?preview=true`），让 LLM 能看到每个文件的内容摘要以判断文书类型。

**查询参数**：
- `preview=true`：为非二进制文件附带内容预览（前 300 字符，空白压缩）

**过滤规则**：自动排除系统文件（`metadata.json`、`llm_cache.json`、`*.jsonl`、`sys_*`）。
...
**重要性**：写入文件（3.3）只是物理存储，不会自动出现在注册表中。此 API 将文件**注册**到 Context 中——赋予它 ID（如 `D01`）、类型（如 `当事人信息`）等属性，使其可被后续 Skill 通过 ID 引用。`CaseDataStore.saveDerived()` 会在 `writeFile` 之后立即调用此 API。
...
**元数据自动注册**：
- `create`/`copy`/`modify` 操作写入文件后，自动在 `metadata.json` 中注册一条记录：ID 为文件名本身，type 为 `用户文档`
- 这确保 `getCaseContext()` 不会将该文件重新注册为 `U##`（未分类文档），从而**避免触发自动分类流程**
- `remove` 操作通过 `deleteFiles` 内部调用 `getCaseContext()` 自动同步注册表
...
├── dsr.md                 # 用户文档 — 通过 Skill_文件操作 创建，ID 为文件名
...
| *(文件名)* | 用户文档 | 用户通过 `Skill_文件操作` 创建的文件，ID 为文件名本身，不参与自动分类 | 是 |
...
`CaseDataStore` TypeScript 类（`shared/utils/case_store.ts`）封装了以上 HTTP API，实现 `ICaseDataStore` 接口。所有框架组件通过构造函数注入 `ICaseDataStore`，不直接调用 HTTP。

| CaseDataStore 方法 | HTTP 请求 | 说明 |
| :--- | :--- | :--- |
...
| *(tasks 管道直接调用)* | `POST /cases/:caseId/files/ops` | 统一文件操作（create/copy/remove/modify），自动注册为用户文档 |
| *(tasks 管道直接调用)* | `POST /cases/:caseId/files/batch-delete` | 按多模式批量删除文件 |
| `getCaseContext()` | `GET /cases/:caseId/context` | 获取完整文件注册表 |
| *(ContextInitializer)* | `GET /cases/:caseId/metadata` | 读取原始 metadata（不触发同步） |
| *(ContextInitializer)* | `PUT /cases/:caseId/metadata` | 整体替换 metadata |
