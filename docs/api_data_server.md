...
**重要性**：写入文件（3.2）只是物理存储，不会自动出现在注册表中。此 API 将文件**注册**到 Context 中——赋予它 ID（如 `D01`）、类型（如 `当事人信息`）等属性，使其可被后续 Skill 通过 ID 引用。`CaseDataStore.saveDerived()` 会在 `writeFile` 之后立即调用此 API。
