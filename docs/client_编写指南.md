...
| 对话历史 | `~/.lexgent/history/{caseNumber}.md` | `localStorage: lexgent_chat_history_{caseNumber}` | 按案件隔离的对话记录，供 Agent 上下文参考 |
...
- **按案件隔离**：每个案件独立存储，使用 `caseNumber` 区分
  - CLI: `~/.lexgent/history/{caseNumber}.md`
  - Web: `localStorage: lexgent_chat_history_{caseNumber}`
- 格式：每行一条，`User:` 和 `Assistant:` 交替
  ```
  User: 你好，我是李明
  Assistant: 您好，李明！有什么可以帮您的吗？
  User: 请分析诉讼时效
  Assistant: 根据本案情况，诉讼时效为三年...
  ```
